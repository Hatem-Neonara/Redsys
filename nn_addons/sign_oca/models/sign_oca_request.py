...
class SignOcaRequest(models.Model):
    ...
    template_id = fields.Many2one("sign.oca.template", string="Template")
    data = fields.Binary(required=True, string="Fichier")
    ...
    signer_id = fields.Many2one(
        ...
        help="Le signataire lié à l'utilisateur actif.",
        string="Signataire",
    )
    ...
    def _ensure_draft(self):
        ...
        if not self.signer_ids:
            raise ValidationError(
                _("Il n'y a pas de signataires, veuillez les remplir avant de le configurer")
            )
        if not self.state == "draft":
            raise ValidationError(_("Vous ne pouvez configurer les demandes qu'à l'état de brouillon"))
    ...
    def action_send(self, sign_now=False, message=""):
        ...
        for signer in self.signer_ids:
            ...
            self.env["mail.thread"].message_notify(
                ...
                subject=_("Nouveau document à signer"),
...
class SignOcaRequestSigner(models.Model):
    ...
    partner_id = fields.Many2one("res.partner", required=True, ondelete="restrict", string="Contact")
    role_id = fields.Many2one("sign.oca.role", required=True, ondelete="restrict", string="Rôle")
    signed_on = fields.Datetime(string="Signé le")
